...
void drawLeg() {
    if (rotate_leg) {
        // Apply the rotation between -30 and +30 degrees
        if (leg_rotation_angle > 30.0f || leg_rotation_angle < -30.0f) {
            // Reverse the direction when limits are reached
            leg_rotation_angle = (leg_rotation_angle > 30.0f) ? 30.0f : -30.0f;
        }
        // Increment or decrement the rotation angle based on the direction
        leg_rotation_angle += (leg_rotation_angle > 0) ? -1.0f : 1.0f;
    }

    glPushMatrix();
        // Rotate the entire leg based on the leg_rotation_angle
        glTranslatef(0.0f, 0.0f, 0.0f);  // Adjust the position if necessary
        glRotatef(leg_rotation_angle, 1.0f, 0.0f, 0.0f);  // Apply rotation to the leg
